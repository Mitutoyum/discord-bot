...
def get_toggle(guild_id: str | int) -> bool:
    return config_manager.get_flag(f'guild.{guild_id}.automod.toggle', False, check_global=False)
...
class AutoMod(CogMixin):
    ...
    @automod.command(description='Enable automod')
    ...
    @automod.command(description='Disable automod')
    ...
    async def set_channel(self, interaction: Interaction, channel: Optional[TextChannel] = None) -> None:
        ...
        await Messenger(interaction).reply(f'automod channel has been set to {channel.mention}')
    ...
    @automod.command(name='unblacklist-word', description='Unblacklist word')
    async def unblacklist_word(self, interaction: Interaction, word: str) -> None:
        unblacklist_word(interaction.guild_id, word)
        await Messenger(interaction).reply(f'`{word}` is no longer blacklisted')

    @automod.command(name='auto-announce', description='Announce to automod channel')
    async def auto_announce(self, interaction: Interaction, toggle: bool, alert: Optional[bool] = False) -> None:
        ...
        await Messenger(interaction).reply(f'`auto-announce` is now {'enabled' if toggle else 'disabled'}')

    @automod.command(name='auto-delete', description='Delete the message contains the word when detected')
    async def auto_delete(self, interaction: Interaction, toggle: bool) -> None:
        config_manager.set_flag(f'guild.{interaction.guild_id}.automod.auto_delete', toggle)
        await Messenger(interaction).reply(f'`auto_delete` has been {'enabled' if toggle else 'disabled'}')
    ...
    async def on_message(self, message: Message) -> None:
        ...
        for word in get_blacklisted_words(message.guild.id):
            if word in message.content:
                if config_manager.get_flag(f'guild.{guild_id}.automod.auto_announce', False, check_global=False) and automod_channel:
                    ...
                    '''
                    pure_content = config_manager.get_flag(f'guild.{guild_id}.automod.auto_announce.alert', False, check_global=False)
                    if pure_content:
                        pure_content = '@here'

                    await ChannelMessenger(automod_channel).send(content, pure_content)
                
                if config_manager.get_flag(f'guild.{guild_id}.automod.auto_delete', False, check_global=False):
