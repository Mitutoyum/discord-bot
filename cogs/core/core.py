...
class Core(Cog, description="A category for core-functions, owner-only"):
    ...
    async def sync(
        ...
        guild_id: Optional[int] = commands.parameter(
            default=None, description="The id of the guild to sync, none will be global"
        ),
    ) -> None:
        synced_commands = await self.bot.tree.sync(guild=self.bot.get_guild(guild_id))  # type: ignore
    ...
    async def desync(
        ...
        await Messenger(ctx).reply(content="All slash commands have been desynchonized")
