...
class Information(Cog):
    ...
    async def help(self, interaction: Interaction, command: Optional[str] = None):
        ...
        prefix = config_manager.get_flag(f'guild.{interaction.guild_id}.prefix', None)
        ...
        if not command:
            ...
            embed.add_field(
                name='> About the bot',
                value=f'- {self.bot.user.name} is a open source multi-functional bot, see more at [github](https://github.com/Mitutoyum/discord-bot)'
            )
            ...
            if not command:
                raise commands.CommandNotFound(f'Command `{old_command}` is not found')
